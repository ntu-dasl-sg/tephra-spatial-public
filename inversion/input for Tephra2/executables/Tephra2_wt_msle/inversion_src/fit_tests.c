...
/********************************************************************************
FUNCTION: msle
DESCRIPTION: The mean square logarithmic error is used as the goodness-of-fit test.
INPUTS: none
OUTPUT:  double, the fit value,
         based on the calulated mass and the observed mass, at each location.
         sum[0..i] ([log_base10(calculated_mass[i]/observed_mass[i])]^2)
********************************************************************************/
double msle(FILE *log_file, int num_pts, POINT *p_all) {

	int i;
  double msle = 0.0, error;
  ...
  for (i=0; i < num_pts; i++) {
    ...
    error = log10(((p_all+i)->observed_mass) + 1) - log10(((p_all+i)->calculated_mass) + 1);
    msle += (p_all+i)->weight* (error*error);
	}
  msle = msle/(num_pts-16);

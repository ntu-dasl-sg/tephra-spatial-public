...
int init_globals(char *config_file) {
  ...
  while (fgets(line, MAX_LINE, in_config) != NULL) {
    ...
    if (!strncmp(token, "TOLERANCE", strlen("TOLERANCE"))) {
      ...
      else if (FIT_TEST == MSE) {
      	fit = mse;
      	fprintf(log_file, "Goodness-of-fit test=[%d]%s\n", FIT_TEST, "mean-squared-error test");
      }
      else if (FIT_TEST == MAPE) {
      	fit = mape;
      	fprintf(log_file, "Goodness-of-fit test=[%d]%s\n", FIT_TEST, "mean-absolute-percentage-error test");
      }
      else if (FIT_TEST == MAE) {
      	fit = mae;
      	fprintf(log_file, "Goodness-of-fit test=[%d]%s\n", FIT_TEST, "mean-absolute-error test");
      }
      else if (FIT_TEST == MSLE) {
      	fit = msle;
      	fprintf(log_file, "Goodness-of-fit test=[%d]%s\n", FIT_TEST, "mean-square-log-error test");
      }
    }
...
easting northing elevation mass weight into a POINTS array.
...
double minimizing_func(double param[]) {
  ...
  /* Every node can now calculate a tephra blanket for their subset of POINTs */
    set_eruption_values(&e, W, gr);
  for (i=0; i < local_n; i++)
    tephra_calc(&e, pt+i, W, &stats, gr);
#ifdef DEBUG

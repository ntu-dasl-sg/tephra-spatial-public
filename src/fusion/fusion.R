##########################################
## R-code to conduct model data fusion 
##########################################
## -- Michele Nguyen; Maricar Rabonza (2022)
##########################################

# Here we interpolate the residuals using 2 kriging techniques: unweighted and weighted kriging.
...
###############
#-- OUTPUT: Raster maps from the kriging interpolation (.RData files).
...
####--------------------------------------
    ...
    #----------
    # Estimate variogram parameters using maximum-likelihood
    # Use Dataset 1 training points.
    #----------
    ...
    #----------
    # Calculate spatial correlation of the fitted spatial model
    #----------
    ...
    #----------
    # Save figure: Variogram model using Maximum-likelihood fit
    #----------
    
    png(file = paste(loss_metric, "_", vgm_model, "_ml_fit.png", sep = ""),
        height = 1200,
        width = 1500,
        res = 300)
    ...
    #----------
    # Save figure: Spatial correlation using Maximum-likelihood fit
    #----------
...
####--------------------------------------
# CHECK FOR DIRECTIONALITY IN THE VARIOGRAM
####--------------------------------------

    #----------
    # Generate directional variograms
    #----------
    ...
    #----------
    png(file = paste(loss_metric, "_", vgm_model, "_directional.png", sep = ""),
        height = 2400,
        width = 3000,
        res = 300)
    par(mfrow = c(2, 2))
...
####--------------------------------------
# LOAD FORWARD MODEL ON A GRID
# Assign appropriate spatial projection
####--------------------------------------
    
    # Read file
    pred.chi2 <- read.table(paste(forward_model, sep = ""))  
    ...
    # Set variable for prediction
    x1_dist <- coordinates(r.pred) # observation locations/coordinates
    colnames(x1_dist) <- c("Easting", "Northing")
    
    
####--------------------------------------
# MODEL-DATA FUSION - UNWEIGHTED
####--------------------------------------
...
####--------------------------------------
# GENERATE RASTER MAPS OF INTERPOLATED RESULTS
####--------------------------------------
    ...
    # Observation locations
    keep_cols <- c("Easting", "Northing", "Observation")
    ...
    uniform_sd <- rasterFromXYZ(data.frame("Easting" = kriging_df[, "Easting"],
                                           ...
                                           "RMSE" = sqrt(mean((res_data$Observation - res_data$Prediction)^2))))
...
####--------------------------------------
# SAVE FILES AND .RDATA
####--------------------------------------
    
png(file = paste(loss_metric, "_", vgm_model, "_spatial_pred_highres.png", sep = ""),
    height = 1700,
    width = 3000,
    res = 300)
...
png(file = paste(loss_metric, "_", vgm_model, "_kriged_resid.png", sep = ""),
    height = 1525,
    width = 1700,
    res = 300)
...
png(file = paste(loss_metric, "_", vgm_model, "_worden_resid.png", sep = ""),
    height = 1525,
    width = 1700,
    res = 300)
...
save(worden_raster, worden_sd_raster, file = "msl_matern_wolden_rasters.RData")